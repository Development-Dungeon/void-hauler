...
namespace PluginMaster.DesignTools.Editor.PrefabWorldBuilder.Scripts
{
    public static class BezierPath
    {
        ...
        {
            var controlPoints = Interpolate(segmentPoints, scales);
        ...
        {
            var controlPoints = new System.Collections.Generic.List<Vector3>();
            ...
            for (int i = 0; i < segmentPoints.Length; i++)
            {
                ...
                else
                {
                    ...
                    var q0 = p1 - scales[i] * tangent * (p1 - p0).magnitude;
                    var q1 = p1 + scales[i] * tangent * (p2 - p1).magnitude;
                    controlPoints.Add(q0);
                    controlPoints.Add(p1);
                    controlPoints.Add(q1);
                }
        ...
        {
            var drawingPoints = new System.Collections.Generic.List<Vector3>();
        ...
        private static System.Collections.Generic.List<Vector3> FindDrawingPoints(int curveIndex, Vector3[] controlPoints)
        {
        ...
        private static int FindDrawingPoints(int curveIndex, float t0, float t1,
            System.Collections.Generic.List<Vector3> pointList, Vector3[] controlPoints, int insertionIndex)
        {
