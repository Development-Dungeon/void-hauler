    ...
    public class ToolProperties : UnityEditor.EditorWindow
    {
        ...
        private void FloorGroup()
        {
            ...
            using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
            {
                ...
                {
                    UnityEditor.EditorGUIUtility.labelWidth = 70;
                    settings.moduleSizeType = (TilesUtils.SizeType)
                        UnityEditor.EditorGUILayout.Popup("Cell size:", (int)settings.moduleSizeType, _cellTypeNames);
                    using (new UnityEditor.EditorGUI.DisabledGroupScope(
                        settings.moduleSizeType != TilesUtils.SizeType.CUSTOM))
                    {
                        settings.moduleSize = UnityEditor.EditorGUILayout.Vector3Field("", settings.moduleSize);
                    }
                }
                using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                {
        ...
        private void WallGroup()
        {
            ...
            using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
            {
                ...
                using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                {
                    UnityEditor.EditorGUIUtility.labelWidth = 70;
                    settings.moduleSizeType = (TilesUtils.SizeType)
                        UnityEditor.EditorGUILayout.Popup("Wall size:", (int)settings.moduleSizeType, _cellTypeNames);
                    using (new UnityEditor.EditorGUI.DisabledGroupScope(
                        settings.moduleSizeType != TilesUtils.SizeType.CUSTOM))
                    {
                        settings.moduleSize = UnityEditor.EditorGUILayout.Vector3Field("", settings.moduleSize);
