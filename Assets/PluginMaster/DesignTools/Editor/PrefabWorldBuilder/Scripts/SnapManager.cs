...
namespace PluginMaster.DesignTools.Editor.PrefabWorldBuilder.Scripts
{
    ...
    public struct GridOrigin
    {
        ...
        public GridOrigin(string name, Vector3 point)
        {
            _name = name;
            _point = point;
        }
    ...
    public class SnapSettings : ISerializationCallbackReceiver
    {
        ...
        private System.Collections.Generic.Dictionary<string, Vector3> _originsDictionary
            = new System.Collections.Generic.Dictionary<string, Vector3>() { { DEFAULT_ORIGIN_NAME, Vector3.zero} };
        ...
        public string selectedOrigin
        {
            ...
            set
            {
                ...
                _origin = _originsDictionary[_selectedOrigin];
                DataChanged();
            }
        }
        ...
        {
            if(_originsDictionary.ContainsKey(name)) _originsDictionary[name] = origin;
            else _originsDictionary.Add(name, origin);
            _selectedOrigin = name;
            DataChanged();
        }
        public bool OriginsDictionaryContains(string name) => _originsDictionary.ContainsKey(name);
        public Vector3 GetOrigin(string name) => _originsDictionary[name];
        ...
        public void SetSelectedOrigin(int index) => selectedOrigin = GetOriginAt(index);
        ...
        public void OnAfterDeserialize()
        {
            ...
            _originsDictionary = _origins.ToDictionary(origin => origin.name, origin => origin.point);
        }
    }
    ...
    public static partial class PWBIO
    {
        ...
        private static Vector3 SnapFloorTilePosition(Vector3 position)
        {
            var origin = SnapManager.settings.origin + SnapManager.settings.rotation
                * (SnapManager.settings.step * 0.5f - Vector3.up * SnapManager.settings.step.y);

            var localPosition = Quaternion.Inverse(SnapManager.settings.rotation) * (position - origin);
            float Snap(float step, float value) => Mathf.Round(value / step) * step;
            var localSnappedPosition = new Vector3( Snap(SnapManager.settings.step.x, localPosition.x), 0f,
                    Snap(SnapManager.settings.step.z, localPosition.z));
            var result = SnapManager.settings.rotation * localSnappedPosition + origin;
            return result;
        }

        private static Vector3 SnapWallPosition(Vector3 position, out AxesUtils.Axis axis, out bool rotateHalfTurn)
        {
            var toolSettings = WallManager.settings;
            var origin = SnapManager.settings.origin
                + SnapManager.settings.rotation * (Vector3.forward * (SnapManager.settings.step.z / 2)
                + Vector3.right * (SnapManager.settings.step.x / 2));
           
            var localPosition = Quaternion.Inverse(SnapManager.settings.rotation) * (position - origin);
            float Snap(float step, float value) => Mathf.Round(value / step) * step;
            var xSnappedToCenter = Snap(SnapManager.settings.step.x, localPosition.x);
            var zSnappedTocenter = Snap(SnapManager.settings.step.z, localPosition.z);
            var cellCenter = new Vector3(xSnappedToCenter, 0f, zSnappedTocenter);
            var centerToPosition = localPosition - cellCenter;
            var xDistance = Mathf.Abs(centerToPosition.x);
            var zDistance = Mathf.Abs(centerToPosition.z);
            ...
            if (xDistance > zDistance)
            {
                ...
                {
                    xSnappedToBorder -= (SnapManager.settings.step.x - WallManager.wallThickness) / 2;
                    rotateHalfTurn = true;
                }
                else xSnappedToBorder += (SnapManager.settings.step.x - WallManager.wallThickness) / 2;
                axis = AxesUtils.Axis.Z;
            }
            else
            {
                if (centerToPosition.z < 0)
                {
                    zSnappedToBorder -= (SnapManager.settings.step.z - WallManager.wallThickness) / 2;
                    rotateHalfTurn = true;
                }
                else zSnappedToBorder += (SnapManager.settings.step.z - WallManager.wallThickness) / 2;
                axis = AxesUtils.Axis.X;
            }

            var yOffset = toolSettings.moduleSize.y / 2;

            var localSnappedPosition = new Vector3(xSnappedToBorder, yOffset, zSnappedToBorder);
            var result = SnapManager.settings.rotation * localSnappedPosition + origin;
            return result;
        }

        private static Vector3 SnapWallPosition(Vector3 startPoint, Vector3 endPoint,
            out AxesUtils.Axis axis, out int cellsCount, out bool rotateHalfTurn)
        {
            var segment = endPoint - startPoint;
            var localSegment = Quaternion.Inverse(SnapManager.settings.rotation) * segment;
            var magnitudeX = Mathf.Abs(localSegment.x);
            var magnitudeZ = Mathf.Abs(localSegment.z);
            axis = magnitudeX > magnitudeZ ? AxesUtils.Axis.X : AxesUtils.Axis.Z;
            float Snap(float step, float value) => Mathf.Round(value / step) * step;
            var origin = SnapManager.settings.origin
                + SnapManager.settings.rotation * (Vector3.forward * (SnapManager.settings.step.z / 2)
                + Vector3.right * (SnapManager.settings.step.x / 2));
            
            var localStartPoint = Quaternion.Inverse(SnapManager.settings.rotation) * (startPoint - origin);
            var localStartXSnappedToCenter = Snap(SnapManager.settings.step.x, localStartPoint.x);
            var localStartZSnappedTocenter = Snap(SnapManager.settings.step.z, localStartPoint.z);
            var localSrtartCellCenter = new Vector3(localStartXSnappedToCenter, 0f, localStartZSnappedTocenter);
  

            var localEndPoint = Quaternion.Inverse(SnapManager.settings.rotation) * (endPoint - origin);
            var localEndXSnappedToCenter = Snap(SnapManager.settings.step.x, localEndPoint.x);
            var localEndZSnappedTocenter = Snap(SnapManager.settings.step.z, localEndPoint.z);
            var localEndCellCenter = new Vector3(localEndXSnappedToCenter, 0f, localEndZSnappedTocenter);

            var localSnappedSegment = localEndCellCenter - localSrtartCellCenter;
            var snappedMagnitudeX = Mathf.Abs(localSnappedSegment.x);
            var snappedMagnitudeZ = Mathf.Abs(localSnappedSegment.z);

            var localXSnappedToBorder = localEndXSnappedToCenter;
            var localZSnappedToBorder = localEndZSnappedTocenter;
            rotateHalfTurn = false;
            if (axis == AxesUtils.Axis.X)
            {
                if (localEndPoint.z > localEndCellCenter.z)
                    localZSnappedToBorder += (SnapManager.settings.step.z - WallManager.wallThickness) / 2;
                else
                {
                    localZSnappedToBorder -= (SnapManager.settings.step.z - WallManager.wallThickness) / 2;
                    rotateHalfTurn = true;
                }
                cellsCount = Mathf.FloorToInt(snappedMagnitudeX / SnapManager.settings.step.x) + 1;
            }
            else
            {
                if (localEndPoint.x > localEndCellCenter.x)
                    localXSnappedToBorder += (SnapManager.settings.step.x - WallManager.wallThickness) / 2;
                else
                {
                    localXSnappedToBorder -= (SnapManager.settings.step.x - WallManager.wallThickness) / 2;
                    rotateHalfTurn = true;
                }
                cellsCount = Mathf.FloorToInt(snappedMagnitudeZ / SnapManager.settings.step.z) + 1;
            }
            if(cellsCount == 1)
            {
                localXSnappedToBorder = localEndXSnappedToCenter;
                localZSnappedToBorder = localEndZSnappedTocenter;

                var centerToPosition = localEndPoint - localEndCellCenter;
                var xDistance = Mathf.Abs(centerToPosition.x);
                var zDistance = Mathf.Abs(centerToPosition.z);
                if (xDistance > zDistance)
                {
                    ...
                    else localXSnappedToBorder += (SnapManager.settings.step.x - WallManager.wallThickness) / 2;
                    axis = AxesUtils.Axis.Z;
                }
                else
                {
                    ...
                    else localZSnappedToBorder += (SnapManager.settings.step.z - WallManager.wallThickness) / 2;
                    axis = AxesUtils.Axis.X;
                }
            }
            var yOffset = WallManager.settings.moduleSize.y / 2;

            var localSnappedPosition = new Vector3(localXSnappedToBorder, yOffset, localZSnappedToBorder);
