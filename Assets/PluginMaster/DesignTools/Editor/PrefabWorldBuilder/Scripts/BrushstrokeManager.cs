...
namespace PluginMaster.DesignTools.Editor.PrefabWorldBuilder.Scripts
{
    ...
    public static class BrushstrokeManager
    {
        ...
        public static void UpdateBrushstroke(bool brushChange = false)
        {
            ...
            else if (ToolManager.tool == ToolManager.PaintTool.FLOOR) UpdateFloorBrushstroke();
            else if (ToolManager.tool == ToolManager.PaintTool.WALL)
                UpdateWallBrushstroke(WallManager.wallLenghtAxis, cellsCount: 1);
        }
        #endregion
        #region LINE
        ...
        private static void UpdateFirstModularBrushstroke(ModularToolBase settings)
        {
            if (PaletteManager.selectedBrush == null) return;
            _brushstroke.Clear();
            var nextIdx = PaletteManager.selectedBrush.nextItemIndex;
            ...
            var angle = AxesUtils.SignedAxis.GetEulerAnglesFromAxes(settings.forwardAxis, settings.upwardAxis);
            var scale = ScaleMultiplier(nextIdx, settings);
            scale.x = Mathf.Abs(scale.x);
            scale.y = Mathf.Abs(scale.y);
            scale.z = Mathf.Abs(scale.z);
            AddBrushstrokeItem(nextIdx, tangentPosition: Vector3.zero, angle, scale, settings);
        ...
        public static void UpdateFloorBrushstroke(bool deleteBox = false)
        {
            if (FloorManager.state == FloorManager.ToolState.FIRST_CORNER)
            {
                UpdateFirstModularBrushstroke(FloorManager.settings);
                return;
            }
            ...
            var cellsCountX = Mathf.RoundToInt(Mathf.Abs(localDiagonal.x / settings.moduleSize.x)) + 1;
            ...
            var cellsCountZ = Mathf.RoundToInt(Mathf.Abs(localDiagonal.z / settings.moduleSize.z)) + 1;
            ...
            _brushstroke.Clear();
            for (int xIdx = 0; xIdx < cellsCountX; ++xIdx)
            {
                var tangent = deltaX * xIdx;
                for (int zIdx = 0; zIdx < cellsCountZ; ++zIdx)
                {
                    ...
                    if (deleteBox)
                    {
                        ...
                        var strokeItem = new BrushstrokeItem(null, cellCenter, additionalAngle,
                            scaleMultiplier: FloorManager.settings.moduleSize, flipX: false, flipY: false, surfaceDistance: 0);
                        _brushstroke.Add(strokeItem);
                    }
                    else
                    {
                        var nextIdx = deleteBox ? 0 : PaletteManager.selectedBrush.nextItemIndex;
                        var scale = localRotation * ScaleMultiplier(nextIdx, settings);
                        scale.x = Mathf.Abs(scale.x);
                        scale.y = Mathf.Abs(scale.y);
                        scale.z = Mathf.Abs(scale.z);
                        AddBrushstrokeItem(nextIdx, cellCenter, angle, scale, settings);
                    }
                }
            }
        }
        #endregion
        #region WALL
        public static void UpdateWallBrushstroke(AxesUtils.Axis segmentAxis, int cellsCount, bool deleteMode = false)
        {
            if (WallManager.state == WallManager.ToolState.FIRST_WALL_PREVIEW || (cellsCount == 1 && !deleteMode))
            {
                UpdateFirstModularBrushstroke(WallManager.settings);
                return;
            }
            var settings = WallManager.settings;
            var diagonal = WallManager.secondPoint - WallManager.firstPoint;
            var localDiagonal = Quaternion.Inverse(SnapManager.settings.rotation) * diagonal;
            Vector3 delta;
            ...
            var firstPoint = WallManager.secondPoint - SnapManager.settings.rotation * (delta * (cellsCount - 1));
            ...
            void AddItem(Vector3 position, AxesUtils.Axis segmentAxis)
            {
                ...
                if (deleteMode)
                {
                    var additionalAngle = (Quaternion.Euler(angle)
                            * Quaternion.Euler(PaletteManager.selectedBrush.eulerOffset)).eulerAngles;
                    var strokeItem = new BrushstrokeItem(null, position, additionalAngle,
                        scaleMultiplier: FloorManager.settings.moduleSize, flipX: false, flipY: false, surfaceDistance: 0);
                    _brushstroke.Add(strokeItem);
                }
                else
                {
                    ...
                    scale.y = Mathf.Abs(scale.y);
                    scale.z = Mathf.Abs(scale.z);
                    AddBrushstrokeItem(nextIdx, position, angle, scale, settings);
                }
            }
